...
public class Board {
    ...
    public Piece findPiece(Class<? extends Piece> type, Color color) {
        for (Piece[] row : grid)
            for (Piece p : row)
                if (p != null && p.getColor() == color && type.isInstance(p)) return p;
        return null;
    }

    public boolean isUnderAttack(Position pos, Color byColor) {
        for (Piece p : getPieces(byColor))
            if (p.getLegalMoves(this).contains(pos)) return true;
        return false;
    }
}
